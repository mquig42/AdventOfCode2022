...
(define (operation str)
  (let ((parsed (string-split str)))
    (λ (old) (quotient ((if (string=? (second parsed) "*") * +)
                        old
                        (if (string=? (third parsed) "old")
                            old
                            (string->number (third parsed)))) 3))))
...
(define (run-n-rounds monkeys rounds)
    ...
    (define (monkey-turn n)
      (cond ((queue-empty? (vector-ref queues n)) (void))
            (else
             ...
             (let* ((new-val ((vector-ref inspects n) elem))
